...
import { fetchAvgResponseTimeWorkflow } from '../services/dataService';
import { DashboardFilters } from '../types';
import InlineChartCard from './InlineChartCard';
import './ChartCard.css';

const AvgResponseTimeWorkflowChart: React.FC<{ filters: DashboardFilters }> = ({ filters }) => {
  const { data, loading, error, refetch } = useChartData(
    () => fetchAvgResponseTimeWorkflow(filters.dateRange, filters.category),
    [filters.dateRange, filters.category]
  );

  const options: Highcharts.Options = {
    chart: { type: 'areaspline', backgroundColor: 'transparent', height: 180 },
    title: { text: '' },
    xAxis: {
    ...
    series: [{
      ...
      fillColor: 'rgba(181, 215, 160, 0.18)'
    }],
